...
var DeathStar = function () {
  ...
  _createClass(DeathStar, [{
    ...
    value: function processElement(data, key) {
      ...
      var manipulate = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
      ...
      if (this.checkKey(key)) {
        ...
        if (this.checkKey(keyArr[0])) {
          ...
          if (internal) {
            ...
            var tempJSX = obj.key === keyMaster ? obj : this.walkChildren(obj.props.children, keyMaster);
    ...
    key: 'updateKeyRender',
    value: function updateKeyRender(tempJsx) {
    ...
    value: function setProps(key, props) {
      ...
      if (this.checkKey(key)) {
        var tempJsx = this.getElement(key, true);
      ...
      if (this.checkKey(key)) {
        if (attributes.ref) {
          ...
          if (this.checkProps(key)) {
            var _tempJsx = this.getElement(key, true);
    ...
    value: function processRemoveChildren(key, index) {
      ...
      if (this.checkKey(key)) {
        var jsxMaster = this.getElement(key, true);
    ...
    key: 'updateAllKeyPropsReferencesToRender',
    value: function updateAllKeyPropsReferencesToRender(elementJSX) {
      var _this6 = this;
      ...
      if (elementJSX.props.children instanceof Array) {
        elementJSX.props.children.forEach(function (child) {
          child = _this6.swapPropsAttr(child, { key: _this6.getId() });
          child = Object.assign({}, child, { key: _this6.getId() });
          if (child.props.children) {
            _this6.updateAllKeyPropsReferencesToRender(child);
          }
        });
